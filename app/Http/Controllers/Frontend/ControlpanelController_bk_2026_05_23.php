...
use App\Models\ControlPanelsMaster;
...
class ControlpanelController extends Controller{
    ...
    //         $controlPanelData->whereRaw("FIND_IN_SET(?, no_of_pumps)",[$request->no_of_pump]);
    //     }

    //     if($request->power_rating){
    //         $controlPanelData->whereRaw("FIND_IN_SET(?, power_rating)",[$request->power_rating]);
    //     }

    //     if($request->voltage){
    //         $controlPanelData->whereRaw("FIND_IN_SET(?, power_supply)",[$request->voltage]);
    //     }

    //     if($request->application){
    //         $controlPanelData->whereRaw("FIND_IN_SET(?, applications)",[$request->application]);
    //     }

    //     if($request->ambient_temp){
    //         $controlPanelData->whereRaw("FIND_IN_SET(?, min_of_ambient_temp)",[$request->ambient_temp]);
    //     }
    ...
    //         $controlPanelData->whereRaw("FIND_IN_SET(?, communication_protocol)",[$request->communication_protocol]);
    //     }
    ...
    //         $controlPanelData->whereRaw("FIND_IN_SET(?, components)",[$request->component]);
    //     }

    //     if($request->enclosure){
    //         $controlPanelData->where('enclosure',$request->enclosure);
    //     }
    ...
    //         if($row->power_rating){
    //             foreach(explode(',',$row->power_rating) as $power){
    //                 $powers[] = ['id'=>$power,'value'=>$power];
    //             }
    //         }

    //         if($row->power_supply){
    //             foreach(explode(',',$row->power_supply) as $voltage){
    //                 $voltages[] = ['id'=>$voltage,'value'=>$voltage];
    //             }
    //         }

    //         if($row->applications){
    //             foreach(explode(',',$row->applications) as $app){
    //                 $applications[] = ['id'=>$app,'value'=>$app];
    //             }
    //         }

    //         if($row->min_of_ambient_temp){
    //             foreach(explode(',',$row->min_of_ambient_temp) as $temp){
    //                 $ambienttemps[] = ['id'=>$temp,'value'=>$temp];
    //             }
    //         }
    ...
    //         if($row->communication_protocol){
    //             foreach(explode(',',$row->communication_protocol) as $protocol){
    //                 $comunicationprotocols[] = ['id'=>$protocol,'value'=>$protocol];
    //             }
    //         }
    ...
    //         if($row->components){
    //             foreach(explode(',',$row->components) as $component){
    //                 $components[] = ['id'=>$component,'value'=>$component];
    //             }
    //         }
    ...
    //     if(isset($request->enclosure) && !empty($request->enclosure)){
    //         if(isset($controlPanelData[0]) && !empty($controlPanelData[0])){
    //             $panel = $controlPanelData[0];
    //             $data['folder_name'] = $panel->folders_name;
    //             $data['file_name'] = $panel->file_name_comes_under_this_folder;
    //             $data['table_name'] = $panel->table_name;
    //             $data['code'] = $panel->code;

    //             $idberOfPump = $request->no_of_pump;
    //             $power = $request->power_rating;
    //             $voltage = $request->voltage;

    //             if(is_numeric($power)){ 
    //                 $columnName = $idberOfPump . 'x' . $power . "__0kwx" . $voltage . 'v';
    //             }else{ 
    //                 $power = str_replace(".", '__', $power);
    //                 $columnName = $idberOfPump . 'x' . $power . "kwx" . $voltage . 'v';
    //             }
    ...
    //                         $enclourse_exist = $this->calculatePriceInItem(
    //                             $request,
    //                             $columnName,
    //                             count($arrayResult),
    //                             $arrayResult[0]
    ...
    //                         $tax = Tax::where('id',1)->first()->amount;
    ...
    //                 }else{
    //                     $data['cp_records_html'] = 'Column not found!';
    //                     $data['enclourse_exist'] = null;
    //                 }

    //             }else{
    //                 $data['cp_records_html'] = 'Table not found!';
    //                 $data['enclourse_exist'] = null;
    //             }
    ...
    */
    ...
    public function searchByArticleCalculatePriceInItem($component, $enclousre, $starterType, $enclousreItem, $columnName,$enclosure_count,$val = []) 
    {
        ...
        if ($component == 2 && $val['brand_code'] == 1) { // component 2 =  Economic
            ...
            $price = $this->getMasterSheetPriceData($val['brand_code'], $val['function_code'], $val['range']) * $val[$columnName]; // A Code: 02-04-2026
        }

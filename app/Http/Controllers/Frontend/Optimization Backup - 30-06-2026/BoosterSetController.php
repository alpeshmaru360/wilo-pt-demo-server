    ...
    */
    ...
    public function addToCart(Request $request) 
    {
        ...
        if(!empty($request->full_article_number) && $request->full_article_number != null)
        {
            ...
            if ($boosterCartCpDetail) { 
                ...
                $request->voltage_id = optional($request_data)->voltage_id;
            ...
            $cpPower = $request->power ?? null;  
            $cpVoltage = $request->voltage ?? null;   
            $cpApplication = $request->application ?? null;
            $cpAmbient_temp = $request->ambient_temp ?? null;
            $cpStater_type = $request->stater_type ?? null;
            $cpCommunication_protocol = $request->communication_protocol ?? null;
            $cpIp_rating = $request->ip_rating ?? null;
            $cpComponent = $request->component ?? null;
            $cpEnclosure = $request->enclosure ?? null;
        ...
        if (!$NewcontrolPanelData) {
            $msg = 'Control Panel Data is not available.';
            return response()->json(array('success' => true, 'msg' => $msg));
        } 
        ...
        if($request->adder_ids || $request->mechanical_adder_ids){
            ...
            if($boosterCartData == null || $request->qoutation_value != null){
                ...
                // dd("case 1: Full Pump with Options",$request->no_of_pump,$request->power,$request->voltage,$request->application,
                //         $request->ambient_temp,$request->stater_type,$request->communication_protocol,
                //         $request->ip_rating,$request->component,$request->enclosure,$request->range);

                // A Code: 25-06-2026 Start
                $fields = [
                    'no_of_pump'             => 'number_of_pumps',
                    'power'                  => 'powers',
                    'voltage'                => 'voltages',
                    'application'            => 'applications',
                    'ambient_temp'           => 'ambient_temps',
                    'stater_type'            => 'starter_types',
                    'communication_protocol' => 'comunication_protocols',
                    'ip_rating'              => 'ip_ratings',
                    'component'              => 'components',
                    'enclosure'              => 'enclousres',
                    'range'                  => 'ranges',
                ];
                ...
                $cpNo_of_pump             = $result['no_of_pump'];
                $cpPower                  = $result['power'];
                $cpVoltage                = $result['voltage'];
                $cpApplication            = $result['application'];
                $cpAmbient_temp           = $result['ambient_temp'];
                $cpStater_type            = $result['stater_type'];
                $cpCommunication_protocol = $result['communication_protocol'];
                $cpIp_rating              = $result['ip_rating'];
                $cpComponent              = $result['component'];
                $cpEnclosure              = $result['enclosure'];
                $cpRange                  = $result['range'];
                // A Code: 25-06-2026 End
                ...
                BoosterCartCpDetail::create([
                    ...
                    'no_of_pump'             => $cpNo_of_pump,
                    'power'                  => $cpPower,
                    'voltage'                => $cpVoltage,
                    'application'            => $cpApplication,
                    'ambient_temp'           => $cpAmbient_temp,
                    'stater_type'            => $cpStater_type,
                    'communication_protocol' => $cpCommunication_protocol,
                    'ip_rating'              => $cpIp_rating,
                    'component'              => $cpComponent,
                    'enclosure'              => $cpEnclosure,
                    'range'                  => $cpRange,
                ]); 

                $request->no_of_pump             = $result['no_of_pump'];
                $request->power                  = $result['power'];
                $request->voltage                = $result['voltage'];
                $request->application            = $result['application'];
                $request->ambient_temp           = $result['ambient_temp'];
                //$request->stater_type            = $result['stater_type'];
                $request->communication_protocol = $result['communication_protocol'];
                $request->ip_rating              = $result['ip_rating'];
                //$request->component              = $result['component'];
                //$request->enclosure              = $result['enclosure'];
                $request->range                  = $result['range'];   
                // A Code: 25-06-2026  End            
                ...
                //here123
                //dd("case 1: getControlPanelDataItemSave full pump with options",$request->cp_id,$request->no_of_pump,$request->power,$request->voltage,$request->range);
                //$this->getControlPanelDataItemSave($request->cp_id, $request, $boosterCartId);                
                $this->getControlPanelDataItemSave($booster_cart_cp_id, $request, $boosterCartId); // A Code: 25-06-2026  
                ...
                if (empty($boosterCartData->quotation_no)) {
                    ...
                    if(!empty($request->full_article_number) && $request->full_article_number != null)
                    {
                        ...
                        $boosterCartId = $boosterCart->id;
                        if($boosterCartCpDetail){
                            // Copy of booster cart CP Detail                             
                            $boosterCartCp_info = $boosterCartCpDetail->replicate();
                            $boosterCartCp_info->booster_cart_id = $boosterCartId;
                        ...
                        // dd("case 2: with article number with Options",$request->no_of_pump,$request->power,$request->voltage,$request->application,
                        // $request->ambient_temp,$request->stater_type,$request->communication_protocol,
                        // $request->ip_rating,$request->component,$request->enclosure,$request->range);                      
                        ...
                        // dd("case 3",$request->no_of_pump,$request->power,$request->voltage,$request->application,
                        // $request->ambient_temp,$request->stater_type,$request->communication_protocol,
                        // $request->ip_rating,$request->component,$request->enclosure,$request->range);

                        BoosterCartCpDetail::create([
                            'booster_cart_id'        => $boosterCart->id,
                            'no_of_pump'             => $request->no_of_pump,
                            'power'                  => $request->power,
                            'voltage'                => $request->voltage,
                            'application'            => $request->application,
                            'ambient_temp'           => $request->ambient_temp,
                            'stater_type'            => $request->stater_type,
                    ...
                    $request->power = $cpPower;
                    $request->voltage = $cpVoltage;
                    $request->range = $cpRange;
                    // A Code: 24-06-2026 End

                    //dd("case 2: getControlPanelDataItemSave search article with options",$request->cp_id,$request->no_of_pump,$request->power,$request->voltage,$request->range);
                    $this->getControlPanelDataItemSave($request->cp_id, $request, $boosterCartId);
                    if ($request->mechanical_adder_ids) {
                        $this->boosterAddersData($boosterCartId, $request);
                    }
                ...
                if($boosterCartData == null || $request->qoutation_value != null){
                    ...
                    $boosterCartId = $boosterCart->id;
                        ...
                        // $bcart = BoosterCart::with('cpDetails')->find($boosterCart->id);
                        // echo $bcart->cpDetails->power;
                        // echo $bcart->cpDetails->voltage;
                        // dd($bcart);
                    ...
                    if (!empty($bill_of_mechanical)) {
                        foreach ($bill_of_mechanical as $data) {
                            ...
                            $this->insertBoosterItem($boosterCartId, $data['brand_code'], $data['function_code'], $data['range'],  
                            $data['qty'], $data['price'], $data);
                        }  
                    }
                    $this->insertItem($boosterCartId, $request->no_of_pump, $request->ptp_distance_id, $request->system_pressure, $request->manifold);
                    //dd("case 3: getControlPanelDataItemSave  search article",$booster_cart_cp_id,$request->cp_id,$request->no_of_pump,$request->power,$request->voltage,$request->range);
                    //$this->getControlPanelDataItemSave($request->cp_id, $request, $boosterCartId);
                    $this->getControlPanelDataItemSave($booster_cart_cp_id, $request, $boosterCartId);// A Code: 25-06-2026
                } else {
                    if(empty($boosterCartData->quotation_no)){
                        ...
                        // A Code: 22-06-2026 Start
                        $booster_original_cart_id = $boosterCartData->id; // A Code: 22-06-2026 
                        $boosterCartCpDetail = BoosterCartCpDetail::where('booster_cart_id', $booster_original_cart_id)->latest('id')->first();
                        ...
                        if (!$boosterCartCpDetail) { 
                            ...
                            $booster_cart_cp_id = $NewcontrolPanelData->id;
                            
                        } else {

                            $booster_cart_cp_id = $boosterCart->cp_id;

                            $request->no_of_pump             = $boosterCartCpDetail->no_of_pump;
                            $request->power                  = $boosterCartCpDetail->power;
                            $request->voltage                = $boosterCartCpDetail->voltage;
                            $request->application            = $boosterCartCpDetail->application;
                            $request->ambient_temp           = $boosterCartCpDetail->ambient_temp;
                            $request->stater_type            = $boosterCartCpDetail->stater_type;
                            $request->communication_protocol = $boosterCartCpDetail->communication_protocol;
                            $request->ip_rating              = $boosterCartCpDetail->ip_rating;
                            $request->component              = $boosterCartCpDetail->component;
                            $request->enclosure              = $boosterCartCpDetail->enclosure;
                        ...
                        // dd("case 5: Full Pump without Options",$booster_cart_cp_id,$request->no_of_pump,
                        //     $request->power,$request->voltage,$request->application,
                        //     $request->ambient_temp,$request->stater_type,$request->communication_protocol,
                        //     $request->ip_rating,$request->component,$request->enclosure,$request->range);
    ...
    {   
        ...
        foreach ($arrayResult as $key => $val) {
            ...
            //Before go the price calculation, Brand code “1” should be replaced by “2” and get the relevant 
    ...
    public function ajaxDetailModalBooster(Request $request) 
    {
        ...
        if ($cpDetails->no_of_pump) {            
            ...
            $application = $cpDetails->application;
            $ambientTemp = $cpDetails->ambient_temp;
            $starterType = $cpDetails->stater_type;
            $communicationProtocol = $cpDetails->communication_protocol;
            $ipRating = $cpDetails->ip_rating;
            $component = $cpDetails->component;
            $enclosure = $cpDetails->enclosure;
            $range = $cpDetails->range;
    ...
    {        
        ...
        if ($booster_cart_id == null)
        {
            $booster_cart_id = DB::table('booster_cp_items')
                        ...
                        ->where('item_description', 'LIKE', '%Enclosure%')
                        ->value('item_description');
            if (!empty($booster_cart_id)) {
                $item_description_height = substr($booster_cart_id, 0, strpos($booster_cart_id, 'Hx'));
                $enclosure_height = explode('Enclosure', $item_description_height);
                $panel_height = trim($enclosure_height[1] ?? '');
                $item_description_width = substr($booster_cart_id, 0, strpos($booster_cart_id, 'Wx'));
                $enclosure_width = explode('Hx', $item_description_width);
                $panel_width = trim($enclosure_width[1] ?? '');
            } else {
                $panel_height = 0;
                $panel_width = 0;
            }

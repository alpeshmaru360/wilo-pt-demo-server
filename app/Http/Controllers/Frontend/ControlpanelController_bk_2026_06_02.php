...
class ControlpanelController extends Controller{
                    ...
                    if($arrayResult){
                        ...
                        else
                        {
                            foreach($arrayResult as $key => $val) {
                                ...
                                $price += $unitPrice;
                                //dd($request->enclousreItem, $columnName, $cpRecords1,$val, $price);
                                //echo "<pre>";print_r($val);echo "</pre>";
                                //var_dump($unitPrice);echo "<br>";
                                // "3", "0.75"
                            }
                            //dd($price);
                            //exit();
                            ...
                            // Uncomment for debug
                            // $rowPrices = []; // store each row price
                            // foreach ($arrayResult as $key => $val) {
                            //     $rowPrice = $this->calculatePriceInItem($request, $columnName, $cpRecords1, $val);
                            //     $price += $rowPrice;
                            //     $rowPrices[] = $rowPrice;
                            //     echo "Row " . ($key + 1) . " => " . $rowPrice . "<br>";
                            // }
                            // echo "<pre>";
                            // print_r($rowPrices);
                            // echo "</pre>";
                            ...
                            //dd($price,$price1,$request->code_price);
                            $tax = Tax::where('id', 1)->get()[0]->amount;
                            $data['cp_price_booster'] = number_format($price, 2);
                            // dd($price,ControlPanel::controlpanel_over_head(), User::ic_margin_control_panel());
    ...
    //     $baseBrand = $val['brand_code'];
    //     $function  = $val['function_code'];
    //     $range     = $val['range'];
    //     $qty       = $val[$columnName] ?? 0;

    //     // =========================
    //     // STEP 1: Enclosure logic
    //     // =========================
    ...
    //             $baseBrand == $enclousreItem['brand_code'] &&
    ...
    //             $unitPrice = $this->getMasterSheetPriceData($baseBrand, $function, $range);
    ...
    //         // Economic
    //         if ($component == 'economic' && $baseBrand == 1) {
    //             $unitPrice = $this->getMasterSheetPriceData(2, $function, $range)
    //                 ?: $this->getMasterSheetPriceData($baseBrand, $function, $range);

    //             return $unitPrice * $qty;
    //         }

    //         // GRP enclosure
    //         if ($enclosure == 'grp' && $baseBrand == 5 && $function == 1) {
    //             $unitPrice = $this->getMasterSheetPriceData(31, 63, $range);
    //             return $unitPrice ? $unitPrice * $qty : 0;
    //         }

    //         // Stainless steel
    //         if ($enclosure == 'stainless steel' && $baseBrand == 5 && $function == 1) {
    //             $unitPrice = $this->getMasterSheetPriceData(5, 64, $range);
    //             return $unitPrice ? $unitPrice * $qty : 0;
    //         }

    //         // METAL + XTREME
    //         if ($enclosure == 'metal' && $baseBrand == 8 && $starter == 'xtreme') {
    //             $unitPrice = $this->getMasterSheetPriceData(32, $function, $range);
    //             return $unitPrice ? $unitPrice * $qty : 0;
    //         }
                    ...
                    case($id >= 37 && $id <= 44):  //electrical_adder_per_pump code
                        ...
                        if ($arrayResult) {
                            foreach ($arrayResult as $key => $val) {
                                ...
                                //     echo "<pre>";print_r($val['brand_code']);echo "</pre>";
                                //     echo "<pre>";print_r($val['function_code']);echo "</pre>";
                                //     echo "<pre>";print_r($val['range']);echo "</pre>";
                                // }                                
                                if($component == 2 && $val['brand_code'] == "1")
                                {
                                    $exist = $this->getMasterSheetPriceData(2, $val['function_code'], $val['range']);
    ...
    //     // Mapping of request fields to their respective lookup tables
    //     $lookups = [
    //         'power_rating'          => 'powers',
    //         'voltage'               => 'voltages',
    ...
    //         $data[$field] = $value 
    //             ? DB::table($table)->where('value', $value)->value('id') ?? 0 
    //             : 0;
    //     }

    //     // Replace request data with updated IDs
    ...
    */
    ...
    // {
    //     $articleNumber = $request->article_number;
    //     $full_article_number_for_stock = $request->full_article_number_for_stock;        
    ...
    //         //$panel = ControlPanelsMaster::where('id', $controlPanelCartData->control_panel_id)->first();
    //         //dd($panel->table_name);
    //         // Search full_article_number_for_stock in Warehouse
    //         $existsArticleNumber = WarehousePumpDetails::where('art_no', $full_article_number_for_stock)->exists();           

    //         // if exist then Update stock_check = 1 Other Wise 0
    //         DB::table('control_panel_carts')
    //             ->where('full_article_number', $articleNumber)
    //             ->update([
    ...
    //             $price = $this->getMasterSheetPriceData(2, $val['function_code'], $val['range']) * $val[$columnName]; //Qty * 
    //         }else {
    ...
    //         }
    //     } else {
    //         // A Code: 02-04-2026 Start
    //         $price = $this->getMasterSheetPriceData($val['brand_code'], $val['function_code'], $val['range']) * $val[$columnName];
    //         // A Code: 02-04-2026 End
    //     }
    //     return $price;
    // }

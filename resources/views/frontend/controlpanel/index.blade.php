            ...
            <div class="formsMidSection">
                ...
                <div class="formWidget">
                    ...
                    <form id='controlpanel_form' action="">
                        ...
                        <div class="formFields" id="full_article_number_for_stock_field">
                            <div class="helpBtnWrap" style="position: relative;">
                                <a href="" class="helpBtn">?</a>
                                <div class="popper-content hide">{{$tool_tip[0]->article_number}}</div>
                            </div>
                            <input type="text" class="formInput" name="full_article_number_for_stock" id="full_article_number_for_stock" placeholder="Full article number for stock">
                        </div>
                        @endif
...
<script>
    ...
    //     function toggleFullArticleField() {
    //         let articleNumber = $('#article_number').val().trim();

    //         if (articleNumber !== '') {
    //             $('#full_article_number_for_stock_field').show();
    //         } else {
    //             $('#full_article_number_for_stock_field').hide();
    //         }
    //     }

    //     // Run on page load
    //     toggleFullArticleField();

    //     // Run on input change
    //     $('#article_number').on('keyup change', function () {
    //         toggleFullArticleField();
    //     });
    // });
...
//    });
    ...
    $(document).on("click", '#addtocart', function (event) 
    {
        ...
        $.ajax({
            ...
            //             'dataType': 'json',
            data: $('#controlpanel_form').serialize() 
                + "&control_panel_id=" + $('#cp-record-data').val() 
                + "&adder_ids=" + adderIds 
                + "&cp_id=" + cp_id
                ...
                // A Code: 07-04-2026 Start
                + "&full_article_number=" + $('#article_number').val(),
                // A Code: 07-04-2026 End
                //contentType: false,
            success: function (response) {
                if (response.url) {
                    location.reload();
    ...
    // A Code: 13-11-2026 Start
    $("#full_article_number_for_stock").bind('keypress', function (e) {
        if(e.which == 13) {
            if ($(this).val().length > 0) {
                var article_number = $('#article_number').val();
                var full_article_number_for_stock = $('#full_article_number_for_stock').val();
                $.ajax({
                    type: "get",
                    url: "{{route('cp.searchbyfullarticlestock')}}",
                    data: {article_number: article_number, full_article_number_for_stock: full_article_number_for_stock},
                    success: function (response) {
                        $("#cp-id").val(response.data.cp_id);                            
                        $("#table-name").val(response.data.table_name);
                        $("#column-name").val(response.data.column_name);

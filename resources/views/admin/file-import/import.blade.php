        ...
        <form action="{{ route('admin.upload') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                ...
                <select name="folder_name" id="folder-name" class="form-control">
                    ...
                    <option value="{{ $folderName->folders_name }}">{{ $folderName->folders_name }}</option>
                    @endforeach
            ...
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                ...
                <select name="file_name" id="file-name" class="form-control h-100">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        //     $.ajax({
        //         url: "{{ URL::to('admin/ajax-rangeName') }}",
        //         type: 'get',
        //         data: {rangeId: rangeId},
        //         dataType: 'json',
        //         success: function (response) {

        //             var len = response.foldersnames.length;
        //             $('#folder-name').empty();
        //             $('#folder-name').prop('disabled', false);
        //             $('#folder-name').append("<option value=''>Select</option>");
        //             for (var i = 0; i < len; i++) {

        //                 var foldername = response.foldersnames[i]['folders_name'];

        //                 $('#folder-name').append("<option value='" + foldername + "'>" + foldername + "</option>");
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    for (var i = 0; i < len; i++) {
                        ...
                        var filename = response.filenames[i]['file_name_comes_under_this_folder'];
